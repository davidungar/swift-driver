...
extension IncrementalCompilationState {
  ...
  public func collectJobsDiscoveredToBeNeededAfterFinishing(
    ...
    return try confinementQueue.sync {
      ...
      assert(Set(invalidatedInputs.inputs).isDisjoint(with: finishedJob.primaryInputs),
  ...
  private func collectInputsInvalidatedByRunning(_ job: Job)-> InvalidatedInputs {
    ...
    return InvalidatedInputs(
        Set(
          job.primaryInputs.flatMap { input in
            collectInputsInvalidated(byCompiling: input).inputs
          }
        )
  ...
  -> InvalidatedInputs {
    ...
    return InvalidatedInputs(skippedCompileGroups.keys)
  }

  /// Find the jobs that now must be run that were not originally known to be needed.
  private func getJobs(for invalidatedInputs: InvalidatedInputs
  ) throws -> [Job] {
    let unbatched = invalidatedInputs.inputs.flatMap { input -> [Job] in
